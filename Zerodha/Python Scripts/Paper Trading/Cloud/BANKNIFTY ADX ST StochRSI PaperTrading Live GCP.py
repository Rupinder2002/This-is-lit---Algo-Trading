# -*- coding: utf-8 -*-
"""
Created on Sun Mar 26 23:37:57 2022
...
def sl_price(ohlc):
    ...
    sl = 3 * ta.atr(ohlc['high'],ohlc['low'],ohlc['close'], 20)[-1]
    return round(sl,1)

def run_strategy(sl_pct = 0.2):
    global ord_df
    ...
    for ticker in tickers:
        ...
        try:
            ...
            ohlc1 = fetchOHLC(ticker = 'NIFTY BANK',interval = "15minute",days = 6)
...
df = df[df["strike"].isin([s2,s3])].reset_index(drop = True)
...
while True:
    now = dt.datetime.now()
    if (now.hour >= 9) and (now.hour <= 15 and now.minute <= 16):
