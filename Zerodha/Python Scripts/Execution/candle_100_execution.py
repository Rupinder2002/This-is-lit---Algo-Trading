...
instrumentLookup(instrument_df,'BANKNIFTY22FEBFUT')
...
# NOTE  
wseries = '22JAN' # <--------------------CHANGE WEEKLY EXPIRY SERIES EVERY FRIDAY----------->
# NOTE
...
while True:
    ...
    if (now.hour >= 9 and now.minute >= 15 ):
        ...
        candle = df.resample('5min').agg({ 'last_price' : ['first','max','min','last'] })  #check if M is minutes or months
        ...
        if in_trade :
            ...
            if check1 :
                stoploss1 = round(target1 - 10,2)
                target1 = round(target1 + 10.3)
                ...
                cepoints = round((trade_dict['traded_price'] - current_traded_price),2)
            ...
            if candle['diff'].iloc[-1]  > 100 and last_candle != candle.index[-1]:
                ...
                tradable_strike_for_today = round(ltpc-50 ,-2)        
                ...
                trade_dict = {"name":name, "status":"CALL",  "strike_price":strike, "quantity":quant, "call_entry_price":ltp1, "traded_time":traded_time, "traded_price":traded_price, "stop_loss_CE":stop_loss1}
            ...
            # elif candle['C'].iloc[-1] - candle['O'].iloc[-1] < -100 :
                ...
                trade_dict = {"name":name, "status":"PUT",  "strike_price":strike, "quantity":quant, "put_entry_price":ltp1, "traded_time":traded_time, "traded_price":traded_price, "stop_loss_PEE":stop_loss1}
