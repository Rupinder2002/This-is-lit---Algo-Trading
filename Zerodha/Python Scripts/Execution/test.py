...
def run_strategy(sl_amt = 50,quantity = 1):
    global ord_df
...
def place_sl_target_order(ticks, target_amt = 100, quantity = 1):
    ...
    for tick in ticks:
        ...
        if ticker in active_tickers:
            ...
            elif last_trade['Order'] == 'buy' and tick['last_price'] >= (price + target_amt):
                placeOrder(ticker, 'sell', quantity)
            ...
            elif last_trade['Order'] == 'sell' and tick['last_price'] <= price - target_amt:
                placeOrder(ticker, 'buy', quantity)
                reason = 'Target Achieved'
                trade = pd.DataFrame([[dt.datetime.now(),ticker,tick['last_price'],None,'buy',reason]],columns = ord_df.columns)
                ord_df = pd.concat([ord_df,trade])
